...
The distinction matters here. Some learners collapse to sending nothing on some
archetypes and score exactly 0.00 there, so a mean over all five archetypes is a
mean over a very lumpy distribution. Pairing the comparison cell by cell is what
keeps that from turning into a false ranking. Which learners those are is read
off the result files and named on screen, never fixed in this prose -- it has
already changed once, when a re-run of the DQN notebook stopped collapsing.
"""
...
# Same configuration switch as the Overview page, reading the same session-state
# key, so the choice a reader makes on one page follows them to the other. Two
# pages silently describing two different runs is the failure this prevents.
configs = data.available_configs()
config_labels = list(configs)
if len(config_labels) > 1:
    stored = st.session_state.get("config_choice")
    # config_labels[0], not [-1]: fall back to the five-seed final run rather
    # than to whatever happens to sit last in CONFIGS.
    default = stored if stored in configs else config_labels[0]
    config = st.sidebar.radio("Configuration", config_labels,
                              index=config_labels.index(default),
                              key="config_choice")
else:
    config = config_labels[0] if config_labels else ""
...
if summary.empty:
    st.warning("No result files yet. Run `python run_all.py` first - "
               "see README.md for the individual commands.")
    st.stop()

if single_seed:
    st.info(
        f"**{config}** - reconstructed from the final learning-curve "
        f"checkpoint, seed 0 only. Paired significance tests below fall back to "
        "pairing over archetypes, because there is only one seed per cell."
    )
...
def _curves(suffix: str):
    ...
    curves = pd.read_csv(c) if c.is_file() else pd.DataFrame()
    conv = pd.read_csv(v) if v.is_file() else pd.DataFrame()
    # These two files are written uppercased by tools/learning_curves.py, so
    # their agent labels do not match theme.AGENT_ORDER on the nose. Normalise
    # once here rather than at each use site -- the previous per-use matching
    # dropped LinUCB from the colour scale while leaving its rows in the data.
    for frame in (curves, conv):
        if not frame.empty and "agent" in frame.columns:
            frame["agent"] = frame["agent"].map(theme.canonical)
    return curves, conv
...
# them would misrepresent whichever question the reader had in mind -- so the
# choice lives in the sidebar and governs this whole page, rather than this
# section carrying a second switch that could disagree with the one above it.
curves, conv = _curves(suffix)

if curves.empty:
    st.info(f"No learning curves recorded for **{config}**. Run "
            "`python tools/learning_curves.py` to populate this section.")
else:
    st.caption(f"Showing the **{config}** run, selected in the sidebar.")
    ...
    domain, rng = theme.domain_range(cur["agent"])
