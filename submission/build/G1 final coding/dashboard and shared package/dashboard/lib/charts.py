...
def leaderboard_bars(board: pd.DataFrame, title: str = "Mean reward per episode",
                     width: int = 470, show_ci: bool = True):
    ...
    `show_ci=False` drops the whiskers and the CI tooltips, for a single-seed
    run where an interval would be fabricated. A zero-width whisker drawn on a
    one-seed mean reads as a precise measurement, which is the opposite of what
    it is; omitting the mark is the honest rendering.
    """
    if board.empty:
        return None
    board = board.copy()
    ...
    bars = base.mark_bar(height=16, cornerRadiusEnd=4).encode(
        ...
        tooltip=[alt.Tooltip("agent:N", title="agent"),
                 ...
                 *([alt.Tooltip("ci_low:Q", title="95% CI low", format=".2f"),
                 ...
                 alt.Tooltip("n:Q", title="agent x seed cells")],
    )
    ci_layers = ([base.mark_rule(strokeWidth=2, color=theme.TEXT_SECONDARY,
                                 opacity=0.55).encode(
    ...
    return alt.layer(zero, bars, *ci_layers, *label_layers).properties(
