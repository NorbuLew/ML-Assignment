...
VARIANTS = {
    ...
    # Coverage guaranteed by construction. Every variant above can still elect
    # to contact nobody, which is what makes their coverage a lottery -- see
    # NightShiftWorker, reached by one seed in three. MinContactAgent removes
    # silence from the feasible set and changes nothing else: the quota decides
    # *whether* a send happens, the policy still decides *when* and *which
    # message*. This is the budgeted-MDP formulation, so the constraint is on
    # the feasible set and never on the reward function.
    "mincontact": dict(hp=TEAMMATE_HP, safe=False, preseed=True,
                       episodes=None, min_sends=1),

    # The combination actually proposed as the final configuration: guaranteed
    # coverage plus the training budget that fixed the undertraining.
    "mincontact_long": dict(hp=TEAMMATE_HP, safe=False, preseed=True,
                            episodes=1500, min_sends=1),
}
...
def build(variant: str, seed: int):
    ...
    if spec["safe"]:
        inner = SafeExplorationDDQN(seed=seed, prior=HOLD_PRIOR,
                                    fatigue_ceiling=FATIGUE_CEILING,
                                    **spec["hp"])
    else:
        inner = DDQNAgent(seed=seed, **spec["hp"])
    if spec.get("min_sends"):
        # Wrapped last, so the quota sees the final action distribution. The
        # wrapper forwards `update` untouched, which is what lets the inner
        # agent learn that a forced send at a bad hour was expensive and a
        # forced send at a good hour paid.
        return MinContactAgent(inner, min_sends=spec["min_sends"], window=24)
    return inner
...
def main() -> int:
    ...
    ap.add_argument("--out-suffix", default="",
                    help="write artifacts/tune_study<suffix>.csv instead of "
                         "overwriting the shipped five-archetype study")
    args = ap.parse_args()
    ...
    out_path = OUT / f"tune_study{args.out_suffix}.csv"
