...
Three of the five users received nothing from every deep RL agent. A results
table showing `0.00` invites two wrong readings -- that the run broke, or that
silence is simply optimal -- and neither is true. This page carries the
...
st.markdown(
    '<p class="lede">Three of five users received nothing. Reward was genuinely '
    'being left on the table, the cause was a collapse of every Q-value rather '
    'than a considered choice, and of seven interventions tested only one '
    'recovered both coverage and profit.</p>',
...
if best.empty:
    ...
    dqn = (learners[learners["agent"] == "DQN"]
           .groupby("archetype")["reward_mean"].mean())

    tbl = best.copy()
    tbl["DQN"] = tbl["archetype"].map(dqn).fillna(0.0)
    tbl["schedule"] = (tbl["best_hour"].map("{:02d}:00".format)
                       + "  " + tbl["action"])
    forfeit = float(tbl[tbl["DQN"] <= 0.01]["reward"].sum())
    ...
    with c1:
        ...
        st.dataframe(
            tbl[["archetype", "schedule", "reward", "ctr", "DQN"]]
            .round(3)
...
if tune.empty:
    ...
    if not ctrl.empty:
        cmp = pd.DataFrame({
            "both": ctrl["reward_mean"],
            best_variant: won["reward_mean"],
