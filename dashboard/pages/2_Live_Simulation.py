...
from cane.persistence import find_checkpoints, load_agent  # noqa: E402
...
# and three of these four are near-silent, which is itself the finding.
#
...
# The reference is always simulated, whatever is selected. A learned policy that
# holds all week is a flat line at zero; on its own that reads as no data, and
# against the baseline it reads as the result it actually is.
show_reference = is_specialist or path_str is not None
...
with s2:
    ...
    if silent:
        ...
        msg += ("\n\nWhether holding is the *right* choice is the Reward "
                "Economics question: a send pays only when the click "
                "probability clears the break-even rate, and every archetype "
                "clears it for a few hours a day.")
        st.info(msg)
    else:
