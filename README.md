...
| Personalisation | the hour each agent chose for each person, against that person's true best hour |
| Diagnosis | why three of five users received nothing, the seven interventions tested, and the one that recovered both coverage and profit |
...
```bash
.venv/Scripts/python.exe tools/render_check.py dashboard/pages/3_Head_to_Head.py
```
